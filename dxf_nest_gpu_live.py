...
def _get_part_candidate(part: 'Part', scale: int, angle: float, mirror: bool, spacing: float,
    ...
    if cand is None:
        ...
        spacing_extra = max(spacing_px - SAFETY_PX, 0) if spacing_px > 0 else 0
        ...
        if spacing_px > 0:
            test_pad, tw, th, test_pad_x, test_pad_y = pad_mask(raw, pw, ph, spacing_extra)
            test = dilate_mask(test_pad, tw, th, spacing_extra)
            tx = test_pad_x - pad_x
            ty = test_pad_y - pad_y
            if tx < 0:
                tx = 0
